...
class ViewController: UIViewController, ARSKViewDelegate, ARSessionDelegate {
    ...
    lazy var classificationRequest: VNCoreMLRequest = {
        do {
            ...
            let request = VNCoreMLRequest(model: model) { request, error in
                ...
                guard let classifications = request.results as? [VNClassificationObservation], error == nil else { return }
    ...
    private func processClassifications(_ classifications: [VNClassificationObservation]) {
        guard let observation = classifications.first else { return }
        self.classification = observation.identifier
        // Show Visually
        
        // Create a transfrom with a translation of 0.2 meters in front of the camera
    ...
    private func classifyImage() {
        ...
        let handler = VNImageRequestHandler(
            cvPixelBuffer: self.currentFrame.capturedImage,
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        self.currentFrame = self.sceneView.session.currentFrame
        self.classifyImage()
    }
